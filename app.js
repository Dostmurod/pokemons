...
let row = document.querySelector('.row')
    ...
    db.setAttribute('class' , 'col-lg-4 col-md-6 col-sm-12')
    db.innerHTML = `<div class="col-lg-12  mt-5 main__block">
    <div class="col-12 bg-light border-2 img__block"><img  class="img__block__img" src="${item.img}" alt="img">
   <div class="col-12 bg-light line"></div>
   <div class="col-12 d-flex card__text">
        <div class="col-7 bg-light mx-3 texts"><h4>${item.name}<br> </h3> <p>Grass, Poison</p></div>
        <div class="col-3 bg-light d-flex justify-content-end fs-4 ml-4 px-3 icons">
            <i class="fa-regular fa-heart mt-4 mr-4 ml-4"></i>
        </div>
   </div>
   <div class="col-10 d-flex justify-content-between align-items-center d-block mx-auto kg">
    <strong>${item.weight}</strong>
    <strong>99 age</strong>
   </div>
</div>
</div>`

$('.searchResuls').appendChild(db)
})
//search
const filterObject=pokemons.filter(e=>{
    return
})
const findFilm = function(name){
    const searchFilm = pokemons.filter((film)=>{
        return film.name.toLowerCase().includes(name)
    })
...
$('#searchFilm').addEventListener('input', (e) =>{
    ...
    const searchTerm =$('#searchFilm').value.trim().toLowerCase()
...
function renderFilms(searchFilm){
    searchFilm.forEach((item) => {
        let row = document.querySelector('.searchResuls')
            let db = document.createElement('div')
            db.setAttribute('class' , 'col-lg-4 col-md-6 col-sm-12')
            db.innerHTML = `<div class="col-lg-12  mt-5 main__block">
            <div class="col-12 bg-light border-2 img__block"><img  class="img__block__img" src="${item.img}" alt="img">
